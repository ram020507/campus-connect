...
import {
  Heart, MessageSquare, Bookmark, BadgeCheck, ImagePlus, Paperclip, Loader2, Send, X, Trash2,
} from "lucide-react";
import { useCampusHub, type PostRole, type CampusPost } from "@/hooks/useCampusHub";
...
type Filter = "mine" | "all" | "student" | "college";
...
const CampusHub = ({ viewer, canPost = true }: CampusHubProps) => {
  ...
  const isMine = (p: CampusPost) =>
    viewer.role === "college"
      ? p.authorRole === "college" && p.collegeName === viewer.collegeName
      : p.authorRole === viewer.role && p.authorId === viewer.key;
    ...
    <div className="grid gap-4 md:grid-cols-[320px_1fr] items-start">
      {canPost && (
        <Card className="md:sticky md:top-4">
          <CardContent className="p-4 space-y-3">
            ...
            <Button className="w-full" onClick={submit} disabled={posting || !title.trim() || !description.trim()}>
      ...
      <div className="space-y-4 min-w-0">
        <div className="flex flex-wrap gap-2">
          {(["mine", "all", "student", "college"] as Filter[]).map((f) => (
        ...
        {hub.loading ? (
          ...
          <Card>
            <CardContent className="p-6 text-center text-sm text-muted-foreground">
              {filter === "mine" ? "You haven't posted anything yet." : "No posts yet."}
            </CardContent>
          </Card>
        ) : (
          visible.map((p) => {
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              This post will be permanently removed from Campus Hub for everyone.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
